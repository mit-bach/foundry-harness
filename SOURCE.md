...
| Agent room | `dependencies.pi-agents-talk-to-each-other` | https://github.com/Timur00Kh/pi-agents-talk-to-each-other | git pin `e4f162a` (`extensions/agent-room/index.ts` only; messenger-swarm is not loaded) |

`foundry-comms.ts` loads `agent-room` from that GitHub pin at install time (messenger-swarm is **not** included).
